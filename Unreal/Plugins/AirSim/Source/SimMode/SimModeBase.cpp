...
ASimModeBase::ASimModeBase()
{
    ...
    static ConstructorHelpers::FClassFinder<AASCameraDirector> camera_director_class(TEXT("Blueprint'/AirSim/Blueprints/BP_CameraDirector'"));
...
void ASimModeBase::initializeCameraDirector(const FTransform& camera_transform, float follow_distance)
{
    ...
    UAirBlueprintLib::FindAllActor<AASCameraDirector>(this, camera_dirs);
    if (camera_dirs.Num() == 0) {
        //create director
        FActorSpawnParameters camera_spawn_params;
        camera_spawn_params.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn;
        camera_spawn_params.Name = "CameraDirector";
        CameraDirector = this->GetWorld()->SpawnActor<AASCameraDirector>(camera_director_class_,
                                                                       camera_transform,
                                                                       camera_spawn_params);
        ...
        CameraDirector = static_cast<AASCameraDirector*>(camera_dirs[0]);
    }
}
